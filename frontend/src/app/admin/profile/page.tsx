...
import { getShopDetails, updateShopDetails, toggleStoreStatus, toggleDeliveryStatus, ShopDetails } from "@/services/shopDetailsService";

export default function AdminProfilePage() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [togglingStatus, setTogglingStatus] = useState(false);
  const [togglingDeliveryStatus, setTogglingDeliveryStatus] = useState(false);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        setIsDeliveryEnabled(details.isDeliveryEnabled ?? true);
  ...
  const handleToggleDeliveryStatus = async () => {
    setTogglingDeliveryStatus(true);
    setError(null);
    try {
      const updated = await toggleDeliveryStatus();
      setIsDeliveryEnabled(updated.isDeliveryEnabled);
    } catch (err: any) {
      setError(err?.message || "Failed to toggle delivery status.");
    } finally {
      setTogglingDeliveryStatus(false);
    }
  };
    ...
    <div className="flex flex-col h-dvh overflow-hidden bg-[#FAF6ED] relative">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-3.5 sm:p-5 max-w-4xl w-full mx-auto flex flex-col gap-5 pb-28">
        <div className="flex flex-col items-center justify-between gap-3">
          ...
          {/* Master Toggles: Store Status & Delivery Orders */}
          <div className="w-full flex flex-wrap items-center gap-2.5 shrink-0">
            {/* Store Status Toggle */}
            <div className="flex-1 flex justify-center items-center gap-2.5 bg-white px-3.5 py-2 rounded-2xl border border-[#E8E1D3] shadow-2xs">
              <div className="flex flex-col">
                <span className="text-[11px] font-extrabold text-[#0B251C] leading-tight">
                  Store Status
                </span>
              ...
              >
                ...
                >
                  {togglingStatus && <Loader2 className="w-2.5 h-2.5 animate-spin text-[#0B392B]" />}
                </span>
              </button>
            </div>

            {/* Delivery Orders Toggle Switch */}
            <div className="flex-1 flex justify-center items-center gap-2.5 bg-white px-3.5 py-2 rounded-2xl border border-[#E8E1D3] shadow-2xs">
              <div className="flex flex-col">
                <span className="text-[11px] font-extrabold text-[#0B251C] leading-tight flex items-center gap-1">
                  <Truck className="w-3 h-3 text-[#0B392B]" />
                  Delivery
                </span>
                <span className={`text-[10px] font-bold ${isDeliveryEnabled ? "text-emerald-600" : "text-amber-600"}`}>
                  {isDeliveryEnabled ? "Accepting" : "Paused"}
                </span>
              </div>
              ...
              <button
                ...
                disabled={togglingDeliveryStatus}
                onClick={handleToggleDeliveryStatus}
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none disabled:opacity-50 ${
                  isDeliveryEnabled ? "bg-emerald-600" : "bg-gray-300"
                }`}
              >
                <span
                  className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out flex items-center justify-center ${
                    isDeliveryEnabled ? "translate-x-5" : "translate-x-0"
                  }`}
                >
                  {togglingDeliveryStatus && <Loader2 className="w-2.5 h-2.5 animate-spin text-[#0B392B]" />}
                </span>
              </button>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-5">
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#E8E1D3] shadow-2xs flex flex-col gap-4">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 flex-wrap gap-2">
                ...
                {/* Delivery Orders Switch Card */}
                <div className="flex items-center gap-3 bg-[#FAF6ED] px-3.5 py-1.5 rounded-2xl border border-[#E8E1D3]">
                  <div className="flex flex-col">
                    <span className="text-[11px] font-extrabold text-[#0B251C] leading-tight flex items-center gap-1">
                      <Truck className="w-3.5 h-3.5 text-[#0B392B]" />
                      Accept Delivery Orders
                    </span>
                    <span className={`text-[10px] font-bold ${isDeliveryEnabled ? "text-emerald-600" : "text-amber-600"}`}>
                      {isDeliveryEnabled ? "Enabled (Customers can order delivery)" : "Disabled (Delivery tab blocked in cart)"}
                    </span>
                  </div>
                  <button
                    type="button"
