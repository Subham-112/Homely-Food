...
import { Utensils, Phone, Lock, ArrowRight, ArrowLeft, CheckCircle2 } from "lucide-react";
...
export default function ForgotPasswordPage() {
  ...
  // Step 1: Verify phone number via /api/user/is-phone-exists
  const handleStep1Submit = async (e: React.FormEvent) => {
    ...
    if (!trimmedPhone) {
      setErrorStep1("Please enter your registered mobile number.");
      return;
    }
    ...
    try {
      const res = await Fetch<{ success: boolean; data?: { exists: boolean }; message?: string }>(
        `/api/user/is-phone-exists?phone=${encodeURIComponent(trimmedPhone)}`
      );

      if (res?.data?.exists === true) {
        setStep(2);
      } else {
        setErrorStep1("Mobile number not registered with us.");
      }
    } catch (err: any) {
      console.error("is-phone-exists check failed:", err);
      setErrorStep1(err?.message || "User not found with this mobile number.");
    } finally {
      setLoadingStep1(false);
    }
  };

  // Step 2: Reset password via /api/user/reset-password
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await Post<{ success: boolean; message: string }>("/api/user/reset-password", {
  ...
  return (
    <div className="min-h-dvh bg-[#FAF6ED] flex flex-col justify-center items-center p-4 sm:p-6 lg:p-8 w-full">
      <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-xl border border-gray-100 p-6 sm:p-8 flex flex-col items-center my-auto relative">
        {/* Header Back Arrow */}
        ...
        <p className="text-xs text-gray-500 text-center mt-1 mb-6">
          {step === 1 ? "Enter your registered mobile number to proceed." : "Set a new password for your account."}
        </p>
          ...
          >
            ...
            <div className="w-1/2 pr-2 shrink-0">
              ...
              <form onSubmit={handleStep1Submit} className="w-full flex flex-col gap-4">
                ...
                <div className="mt-2">
                  <Button type="submit" variant="maroon" disabled={loadingStep1}>
                    {loadingStep1 ? "Checking..." : "Continue"}
                  </Button>
                </div>
              </form>
            </div>
            ...
            <div className="w-1/2 pl-2 shrink-0">
              {/* Display Phone Number at Top */}
              <div className="bg-[#FAF6ED] border border-[#E8E1D3] p-3 rounded-xl text-center mb-4 text-xs">
                <span className="text-gray-500 font-medium">Account Mobile: </span>
                <span className="font-bold text-[#0B251C] font-mono">{mobileNumber}</span>
              </div>
              ...
              <form onSubmit={handleStep2Submit} className="w-full flex flex-col gap-4">
                ...
                <div className="mt-2 flex gap-2">
                  ...
                  <Button type="submit" variant="primary" disabled={loadingStep2 || !!successMsg}>
                    {loadingStep2 ? "Saving..." : "Save New Password"}
                  </Button>
                </div>
              </form>
            </div>
