...
import { Star, Clock, Flame, Layers } from "lucide-react";
...
interface TodaysSpecialSectionProps {
  ...
  addToCart: (item: CartMenuItem, variant?: { id: string; label: string; price: number }) => void;
...
export default function TodaysSpecialSection({
  ...
  const { cart } = useCart();
  const [selectedVariantItem, setSelectedVariantItem] = useState<CartMenuItem | null>(null);
  ...
  return (
    <>
                  ...
                  <div className="flex items-center justify-between gap-2 pt-2 border-t border-gray-100 mt-1">
                    {hasMultipleVariants ? (
                      <span className="text-[10px] font-bold text-gray-400 flex items-center gap-1">
                        <Layers className="w-3 h-3 text-[#0B392B]" />
                        Customisable
                      </span>
                    ) : (
      ...
      {/* Variant Selection Modal for Specials */}
      {selectedVariantItem && (
        <VariantSelectionModal
          isOpen={Boolean(selectedVariantItem)}
          onClose={() => setSelectedVariantItem(null)}
          item={selectedVariantItem}
          cart={cart}
          onAddToCart={(itm, v) => addToCart(itm, v)}
          onUpdateQuantity={(id, delta, vId) => updateQuantity(id, delta, vId)}
        />
      )}
    </>
  );
